...
class MenuButton(Button):
    """Botão do menu principal com feedback de cor: verde (normal) e azul (pressionado)."""
    COLOR_NORMAL = (0, 200, 0)
    COLOR_PRESSED = (0, 0, 200)

    def __init__(self, rect, label, on_click):
        super().__init__(rect, label, on_click)
        self.pressed = False

    def draw(self, screen, font):
        fill_color = self.COLOR_PRESSED if self.pressed else self.COLOR_NORMAL
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            self.pressed = False
            if self.rect.collidepoint(event.pos) and was_pressed:
                self.on_click()
...
class InputField:
    def __init__(self, x, y, w, h, placeholder="", text=""):
        ...
        self.text = text
        self.focus = False
        # Wallpaper / marca d'água opcional
        self.bg_surface = None   # pygame.Surface já carregada
        self.bg_alpha = 80       # transparência padrão

    def set_wallpaper(self, surf, alpha=80):
        self.bg_surface = surf
        self.bg_alpha = alpha

    def draw(self, screen, font):
        # fundo com papel de parede opcional
        if self.bg_surface:
            img = pygame.transform.smoothscale(self.bg_surface, (self.rect.width, self.rect.height))
...
class BiblioApp:
    ...
    # ---------- Papel de parede para inputs ----------
    def _load_input_wallpaper(self):
        # Procura um arquivo de papel de parede padrão
        path = os.path.join("assets", "wallpapers", "input_bg.png")
    ...
    def _apply_wallpaper_to_fields(self, fields_dict, surf):
        if not surf: return
        for f in fields_dict.values():
            if hasattr(f, "set_wallpaper"):
                f.set_wallpaper(surf, alpha=80)
    ...
    def _build_navbar(self):
        W = self.screen.get_width()  # retorna o tamanho da janela
        margin = 20                  
        gap = 10                     
        y = 22
        h = 36
        ...
        n = len(labels)
        # largura de cada botão para caber exatamente no espaço disponível
        w = max(80, (W - 2*margin - (n - 1)*gap) // n)

        self.buttons = []
        x = margin
        for key, text in labels:
            self.buttons.append(
                MenuButton((x, y, w, h), text, lambda k=key: self._go(k))
            )
            x += w + gap
    ...
    def _add_build_fields(self):
        base_x, base_y = 40, 200
        w, h = 420, 40
        self.add_inputs = {
            "name": InputField(base_x, base_y, w, h, placeholder="Nome do item")
        }
        # aplica papel de parede (se existir)
        surf = self._load_input_wallpaper()
        self._apply_wallpaper_to_fields(self.add_inputs, surf)
    ...
    def _user_build_fields(self):
        ...
        surf = self._load_input_wallpaper()
        self._apply_wallpaper_to_fields(self.user_inputs, surf)
    ...
    def _manage_users_toolbar_rects(self):
        ...
        # Configuração menor para não atrapalhar o campo de nome
        btn_h = 36
        pad = 10
        btn_w = 120
        total = 4 * btn_w + 3 * pad
        if total > sw - 80:
            btn_w = max(100, (sw - 80 - 3 * pad) // 4)
            total = 4 * btn_w + 3 * pad
        x0 = sw - 40 - total
        y0 = 198  # um pouco acima para caber o placeholder
    ...
    def render_navbar(self):
        pygame.draw.rect(self.screen, (250, 250, 252), (0, 0, WIDTH, 80))
        pygame.draw.line(self.screen, BTN_BORDER, (0, 80), (WIDTH, 80), width=2)
